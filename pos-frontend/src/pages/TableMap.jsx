...
import { Table2, Clock, Users, RefreshCw, CheckCircle, Loader } from 'lucide-react'
...
const STATUS_COLOR = {
  pending:   { bg: 'bg-amber-500',   ring: 'ring-amber-300',   label: 'Интизор' },
  preparing: { bg: 'bg-blue-500',    ring: 'ring-blue-300',    label: 'Тайёр мешавад' },
  ready:     { bg: 'bg-emerald-500', ring: 'ring-emerald-300', label: 'Тайёр!' },
}
...
export default function TableMap() {
  ...
  const freeCount = tables.filter((t) => !tableOrders[t]).length
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between animate-fade-up">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Нақшаи зал</h1>
          <p className="text-sm text-gray-400">Ҳолати мизҳо дар вақти воқеӣ</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-4 text-sm">
            <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-gray-200" /><span className="text-gray-500">Холӣ: {freeCount}</span></div>
            <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-amber-400" /><span className="text-gray-500">Банд: {occupiedCount}</span></div>
          </div>
      ...
      <div className="flex gap-3 flex-wrap animate-fade-up">
        {[
          { color: 'bg-gray-100 border-gray-200', label: 'Холӣ' },
          { color: 'bg-amber-100 border-amber-300', label: 'Интизори ошхона' },
          { color: 'bg-blue-100 border-blue-300', label: 'Тайёр мешавад' },
          { color: 'bg-emerald-100 border-emerald-300', label: 'Тайёр — хизмат расонед' },
        ].map(({ color, label }) => (
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-xs font-bold ${order ? 'text-gray-600' : 'text-gray-400'}`}>Миз</span>
                  {order && (
                ...
                {order ? (
                  ...
                  <p className="text-[10px] text-gray-300 font-medium">Холӣ</p>
                )}
              </button>
            )
          })}
          ...
          <div className="bg-white rounded-2xl border-2 border-indigo-200 shadow-lg p-5 animate-scale-in">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-black text-gray-800 text-lg">Миз {selected} — Фармоиш #{order.order_number}</p>
                  <p className={`text-sm font-semibold ${sc.bg.replace('bg-', 'text-').replace('500', '700')}`}>{sc.label}</p>
                </div>
              </div>
              <div className="text-right text-sm text-gray-400">
                <p>{fmt(order.created_at)}</p>
                <p className="font-bold text-gray-600">{elapsed(order.created_at)} гузашт</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-500">Кассир: {order.cashier_name}</span>
              <span className="font-bold text-lg text-gray-800">{Number(order.total_amount).toLocaleString('ru')} сом</span>
            </div>
          </div>
        )
      })()}
      ...
      {selected && !tableOrders[selected] && (
        <div className="bg-white rounded-2xl border-2 border-gray-100 p-6 text-center animate-scale-in">
          ...
          <p className="font-semibold text-gray-700">Миз {selected} — Холӣ</p>
          <p className="text-sm text-gray-400 mt-1">Ҳеч фармоише нест</p>
        </div>
      )}
    </div>
  )
